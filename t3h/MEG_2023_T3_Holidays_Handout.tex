...
\begin{quote}
	\textbf{Amendment, 24 September 2023:} The following proof is \textbf{wrong}. 
The way the inequalities are simplified works only when integers are
added to both ends. It is also not true that, as I will apparently
prove, the parity of the number of numbers in every interval we are concerned
with is the same as the parity of the numbers in the interval \((r/2a,r/a)\);
for that would mean when the number of intervals is even, the parity
of all the numbers in all the intervals is always even. This is plainly
false, as if it were true \(5\) would be a quadratic residue modulo \(p\)
regardless of the residue class of \(p\) modulo \(20\). 

I could silently replace what follows by the correct
proof, but I have left it to serve as an illustration that mathematics
demands the most exacting and relentless eye, and that mistakes
are inevitable. The correct proof is attached at the end.

Be assured the conjecture itself is true. 
The problem is in the proof, and thus the fault is mine, not Euler's.
\end{quote}
